...
public interface LetterRepository extends CrudRepository<Letter, Long> {
    ...
    @Query("SELECT new center.unit.letter.presentation.letter.dto.response.MyLetterResponse( u.phone_number AS phone_number, l.type, l.medium_type, " +
            ...
            "    SUBSTRING(l.text FROM :userId FOR 25) AS preview_text, " +
            "    CASE WHEN l.from_id = :userId THEN 'OUT' " +
            "        WHEN l.to_id = 1 THEN 'IN' " +
            "        ELSE NULL " +
    ...
    List<MyLetterResponse> findAllByUserId(Long userId);
}
