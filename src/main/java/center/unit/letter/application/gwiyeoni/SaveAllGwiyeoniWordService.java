...
@Service
public class SaveAllGwiyeoniWordService {
    ...
    public void execute() {
        ...
        while (nestedTimes < 3) {
            ...
            savedNewly = false;
            String[] cyworldText = cyworldService.execute(allWordString).split(",");

            for (int i = 0; i < allWords.length; i++) {
                if (!allWords[i].equals(cyworldText[i]) &&
                        !savedString.contains(cyworldText[i])) {
                    savedNewly = true;
                    savedString.add(cyworldText[i]);
                    gwiyeoniRepository.save(
                            new Gwiyeoni(allWords[i], cyworldText[i])
                    );
                }
            }
        }
